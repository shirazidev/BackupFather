"""Telegram destination — delivers backups via the Bot API sendDocument."""
...
class TelegramDestination(BackupDestination):
    name = "telegram"
    ...
    def _deliver_chunked(self, artifact: BackupArtifact, caption: str) -> DeliveryResult:
        ...
        for part in split_file(artifact.path, self.max_upload_bytes):
            try:
                ...
                self._client.send_document(part.path, part_caption)
                sent += 1
            finally:
